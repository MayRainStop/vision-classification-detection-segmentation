...
\begin{abstract}
本文围绕三类典型视觉识别问题展开实验：细粒度花卉图像分类、道路车辆检测与跟踪、场景语义分割。花卉分类实验使用 Oxford 102 Flowers 数据集，系统比较 ResNet baseline、ImageNet 预训练与从零训练、SE/CBAM 注意力模块、ViT/Swin Transformer 以及 ConvNeXt 等现代卷积网络；最终 ConvNeXt-Tiny 在 384px 输入和 10 个随机种子下取得 \accpm{95.60}{0.61} 的测试准确率。车辆检测实验使用 YOLOv8s 在道路车辆数据集上训练检测器，验证集 mAP50 为 0.520，并结合 ByteTrack 完成跟踪 ID 显示、越线计数和遮挡失败分析。语义分割实验从零实现轻量 U-Net，在 Stanford Background / ICCV09 风格数据上比较 Cross-Entropy、Dice 与组合损失，组合损失取得最高验证集 mIoU 0.6531。实验结果表明，预训练特征、注意力机制、输入分辨率、检测稳定性和区域重叠损失都会显著影响视觉识别系统的最终泛化性能。
\end{abstract}
...
语义分割实验使用 \code{iccv09Data} 中的 \code{labels/*.regions.txt} 作为语义标签，共包含 8 个类别：\code{sky}、\code{tree}、\code{road}、\code{grass}、\code{water}、\code{building}、\code{mountain} 和 \code{foreground\_object}。与图像分类和目标检测不同，语义分割需要为每一个有效像素预测类别，因此模型既要理解全局场景语义，也要恢复局部边界和区域形状。标签中的负数像素表示未知区域，训练时统一映射为 \code{ignore\_index = 255}，损失与指标统计时忽略这些像素。实验中输入图像统一缩放为 \(320 \times 240\)，并使用固定随机种子从全部样本中随机抽取 15\% 作为验证集。

图 \ref{fig:segmentation-dataset-example} 给出了输入图像与语义标签样例。可以看到，天空、道路和建筑等大面积类别占据了多数像素，而 \code{foreground\_object}、\code{mountain} 等类别面积较小，更容易受分辨率和下采样影响。因此，分割实验不能只依赖 pixel accuracy，还需要使用 \code{mIoU} 和 per-class IoU 来衡量区域边界和小类别恢复质量。
...
图 \ref{fig:segmentation-confusion-matrices} 给出了三组最佳模型对应的混淆矩阵。可以看出，\code{sky} 和 \code{building} 的识别最稳定，而 \code{mountain}、\code{water} 和部分前景目标更容易与相邻区域混淆，这也解释了这些类别 IoU 偏低的原因。
