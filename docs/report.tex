...
\begin{center}
    ...
    \vspace{0.4em}
    {\zihao{3}\bfseries 花卉图像分类、道路车辆检测与图像分割\par}
\end{center}
...
\begin{center}
    \small
    GitHub 项目地址：\url{https://github.com/MayRainStop/vision-classification-detection-segmentation}

    模型权重网盘地址：\url{https://drive.google.com/drive/folders/11rpbLTSrK4TePYEZGxlLH_xuVx3fbTB9?usp=sharing}
\end{center}

\vspace{0.6em}

\begin{abstract}
本文围绕三类典型视觉识别问题展开实验：细粒度花卉图像分类、道路车辆检测与跟踪、图像分割。花卉分类实验使用 Oxford 102 Flowers 数据集，系统比较 ResNet baseline、ImageNet 预训练与从零训练、SE/CBAM 注意力模块、ViT/Swin Transformer 以及 ConvNeXt 等现代卷积网络；最终 ConvNeXt-Tiny 在 384px 输入和 10 个随机种子下取得 \accpm{95.60}{0.61} 的测试准确率。车辆检测实验使用 YOLOv8s 在道路车辆数据集上训练检测器，验证集 mAP50 为 0.520，并结合 ByteTrack 完成跟踪 ID 显示、越线计数和遮挡失败分析。图像分割实验从零实现轻量 U-Net，在 Stanford Background / ICCV09 风格数据上比较 Cross-Entropy、Dice 与组合损失，组合损失取得最高验证集 mIoU 0.6531。实验结果表明，预训练特征、注意力机制、输入分辨率、检测稳定性和区域重叠损失都会显著影响视觉识别系统的最终泛化性能。
\end{abstract}

\section{引言}

深度学习视觉系统通常可以分为图像级识别、目标级检测跟踪和像素级语义理解三个层次。图像分类关注整张图片属于哪个类别；目标检测需要在图像或视频中定位多个实例；图像分割则要求对每个像素进行类别判定。三类问题对应不同的监督信号、模型结构和评价指标，也体现了深度神经网络在空间智能问题中的不同建模方式。

本文实验分为三个部分。第一部分研究 Oxford 102 Flowers 细粒度图像分类，该数据集每类训练样本很少，适合分析 ImageNet 预训练、注意力模块和现代 backbone 对小样本分类的影响。第二部分研究道路车辆检测与跟踪，重点是 YOLOv8 检测器、ByteTrack 轨迹维持和遮挡场景下的 ID 跳变。第三部分研究图像分割，使用手写 U-Net 比较不同损失函数对 mIoU 的影响。

本文的写作重点放在实验方法、训练设置、定量结果和结果分析上。检测结果使用 precision、recall 和 mAP；分割结果使用 pixel accuracy 和 mIoU。
...
预训练消融实验的结论非常明显。采用 ImageNet 权重时，两个 ResNet 的测试准确率接近 90\%；从零训练时仅约 53\%--55\%。具体而言，ResNet-18 从 52.72\% 提升到 89.97\%，ResNet-34 从 55.44\% 提升到 89.58\%。两组差距均超过 34\%。
...
\begin{table}[H]
    ...
    \begin{tabular}{lrrr}
        ...
        ResNet-18-SE & 93.63\% & 90.65\% & +0.68\% \\
        ResNet-18-CBAM & 94.12\% & 90.86\% & +0.89\% \\
        ResNet-34 & 92.65\% & 89.58\% & -- \\
        ResNet-34-SE & 93.43\% & 89.72\% & +0.14\% \\
        ResNet-34-CBAM & 93.92\% & 92.15\% & +2.57\% \\
        \bottomrule
    \end{tabular}
\end{table}
...
从完整网格搜索和补充实验可以观察到以下规律。为避免正文表格过于冗长，基础网格中各模型在不同学习率、weight decay、batch size 和 label smoothing 下的完整验证集准确率汇总见附录 \ref{app:classification-hparam-tables}；最终 ConvNeXt-Tiny 384px 主结果配置见附录 \ref{app:classification-final-config}。
...
图 \ref{fig:classification-misclassified-examples} 进一步列出了一组代表性错分样例，并加入少量第 97 类正确识别样本作为参照；图中带有 ref 的条目表示第 97 类参考样本。可以看到，错分通常不是由图像完全失真导致的，而是与花瓣形状、花心结构、颜色组合、拍摄角度和遮挡等细节有关。例如第 96 类多次被预测为第 97 类，样例中花瓣轮廓和颜色分布非常接近；第 50 类与第 12 类的混淆则体现出黄色花卉在局部纹理和花型相似时仍然具有较强挑战性。这说明即使最终模型已经达到较高准确率，Oxford 102 的剩余错误仍主要来自细粒度视觉差异，而不是训练过程没有收敛或模型整体表达能力不足。
...
\begin{figure}[htbp]
    ...
    \caption{最终 ConvNeXt-Tiny 384px 模型的错分样例与第 97 类参考样本；T 表示真实类别，P 表示预测类别，ref 表示正确识别的第 97 类参照样本}
...
图像分割实验使用 \code{iccv09Data} 中的 \code{labels/*.regions.txt} 作为语义标签，共包含 8 个类别：\code{sky}、\code{tree}、\code{road}、\code{grass}、\code{water}、\code{building}、\code{mountain} 和 \code{foreground\_object}。与图像分类和目标检测不同，图像分割需要为每一个有效像素预测类别，因此模型既要理解全局场景语义，也要恢复局部边界和区域形状。标签中的负数像素表示未知区域，训练时统一映射为 \code{ignore\_index = 255}，损失与指标统计时忽略这些像素。实验中输入图像统一缩放为 \(320 \times 240\)，并使用固定随机种子从全部样本中随机抽取 15\% 作为验证集。

图 \ref{fig:segmentation-dataset-example} 给出了输入图像与语义标签样例。可以看到，天空、道路和建筑等大面积类别占据了多数像素，而 \code{foreground\_object}、\code{mountain} 等类别面积较小，更容易受分辨率和下采样影响。因此，图像分割实验不能只依赖 pixel accuracy，还需要使用 \code{mIoU} 和 per-class IoU 来衡量区域边界和小类别恢复质量。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\textwidth]{../tasks/task3_semantic_segmentation/outputs/segmentation_dataset_label_example.png}
    \caption{图像分割数据与标签样例}
    \label{fig:segmentation-dataset-example}
\end{figure}

模型采用手写轻量 \code{U-Net}，包括编码器下采样路径、瓶颈层特征提取、解码器上采样、\code{Skip Connection} 和最终 \code{1x1 Conv} 像素分类头。编码器逐层降低空间分辨率、提取高层语义信息；解码器逐层恢复分辨率并结合浅层特征，从而兼顾整体语义理解和边界细节恢复。
...
\begin{table}[H]
    ...
    \begin{tabular}{ll}
        ...
        \midrule
        Model & U-Net \\
        Input size & \(320 \times 240\) \\
        Epochs & 100 \\
        Batch size & 8 \\
        Learning rate & \(3\times10^{-4}\) \\
        Optimizer & AdamW \\
        Weight decay & \(1\times10^{-4}\) \\
        Validation split & Random 15\% \\
        Seed & 42 \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item 手写 Dice Loss：直接优化预测区域与真实区域的重叠程度；
    \item Cross-Entropy + Dice：兼顾像素级分类稳定性与区域级重叠优化。
\end{itemize}

交叉熵损失对每个有效像素进行多分类监督，其形式为
\begin{equation}
    \mathcal{L}_{CE} = - \frac{1}{|\Omega|}\sum_{i \in \Omega}\log p_{i,y_i},
\end{equation}
其中 \(\Omega\) 表示非忽略像素集合，\(y_i\) 为像素 \(i\) 的真实类别，\(p_{i,y_i}\) 为模型对真实类别的预测概率。交叉熵训练稳定，但当类别面积差异较大时，大面积类别会在损失中占据更高权重。
...
\end{equation}
最终损失写为 \(1-\frac{1}{C}\sum_c Dice_c\)。组合损失使用 \(\mathcal{L}_{CE}+\mathcal{L}_{Dice}\)，希望同时保留交叉熵的分类稳定性和 Dice 的区域重叠优化能力。

\subsection{分割结果}

三种损失函数在验证集上的最佳结果如表 \ref{tab:segmentation-loss-results} 所示。当前实验中，\textbf{Cross-Entropy + Dice} 取得了最高的验证集 \code{mIoU=0.6531}；Dice Loss 次之，为 0.6477；单独使用 Cross-Entropy 为 0.6436。三者的 pixel accuracy 都较高，但 \code{mIoU} 的差异更能反映小类别与边界区域的恢复质量。
...
为了进一步分析不同损失函数对各类别的影响，表 \ref{tab:segmentation-per-class-iou} 给出了验证集 per-class IoU。可以看到，\code{sky}、\code{road} 和 \code{building} 是相对容易的类别，而 \code{mountain} 是最难的类别。组合损失在大多数类别上都取得了最优或接近最优的表现，因此整体 \code{mIoU} 最高。
...
\begin{table}[H]
    ...
    \begin{tabular}{lccccccccc}
        \toprule
        Method & sky & tree & road & grass & water & building & mountain & foreground\_object & Val mIoU \\
        \midrule
...
\subsubsection{单组训练曲线与总体对比}

图 \ref{fig:segmentation-miou-compare} 给出了三种损失函数在验证集上的 \code{mIoU} 对比曲线。Cross-Entropy 在前期收敛较快，但后期提升有限；Dice 在中后期继续改善区域重叠质量；组合损失则在稳定性和最终效果之间取得了更好的平衡，最终达到最高验证集 \code{mIoU}。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\textwidth]{../tasks/task3_semantic_segmentation/runs/val_miou_comparison.png}
    \caption{三种损失函数的验证集 mIoU 对比曲线}
    \label{fig:segmentation-miou-compare}
\end{figure}

图 \ref{fig:segmentation-training-curves} 展示了三组实验各自的训练曲线。三种方法都能稳定收敛，但组合损失在训练集和验证集两侧都表现出更好的综合平衡性。

\begin{figure}[H]
    \centering
    \begin{subfigure}{0.32\textwidth}
        \includegraphics[width=\textwidth]{../tasks/task3_semantic_segmentation/runs/ce/training_curves.png}
        \caption{Cross-Entropy}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.32\textwidth}
        \includegraphics[width=\textwidth]{../tasks/task3_semantic_segmentation/runs/dice/training_curves.png}
        \caption{Dice}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.32\textwidth}
        \includegraphics[width=\textwidth]{../tasks/task3_semantic_segmentation/runs/combo/training_curves.png}
        \caption{Cross-Entropy + Dice}
    \end{subfigure}
    \caption{三种损失函数的单组训练曲线}
    \label{fig:segmentation-training-curves}
\end{figure}

图 \ref{fig:segmentation-confusion-matrices} 给出了三组最佳模型对应的混淆矩阵。可以看出，\code{sky}、\code{road} 和 \code{building} 的识别效果最好，这几个类别在混淆矩阵对角线上的响应最强，说明模型能够较稳定地恢复它们的大面积区域结构。其中，天空通常位于图像上方且外观特征明显；道路和建筑在场景中也往往具有较稳定的空间位置与纹理模式，因此更容易学习。相比之下，\code{mountain} 是最难识别的类别，其 IoU 最低。一个可能的原因是山体在数据集中出现频率较低、像素占比偏小，并且在颜色和纹理上容易与 \code{tree}、\code{grass} 等自然场景区域混淆，因此模型更难为其学习到稳定且清晰的判别边界。

\begin{figure}[H]
    \centering
    \begin{subfigure}{0.32\textwidth}
        \includegraphics[width=\textwidth]{../tasks/task3_semantic_segmentation/runs/ce/best_confusion_matrix.png}
        \caption{Cross-Entropy}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.32\textwidth}
        \includegraphics[width=\textwidth]{../tasks/task3_semantic_segmentation/runs/dice/best_confusion_matrix.png}
        \caption{Dice}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.32\textwidth}
        \includegraphics[width=\textwidth]{../tasks/task3_semantic_segmentation/runs/combo/best_confusion_matrix.png}
        \caption{Cross-Entropy + Dice}
    \end{subfigure}
    \caption{三种损失函数对应的最佳混淆矩阵}
    \label{fig:segmentation-confusion-matrices}
\end{figure}

\subsubsection{样例预测分析}

图 \ref{fig:segmentation-single-predictions} 单独列出了样本 \code{0000382} 在三种损失函数下的预测结果。该样本同时包含天空、建筑、道路/地面、树木和前景目标，能够较直观地观察模型对大面积区域与小目标区域的处理差异。Cross-Entropy 更容易偏向主导类别；Dice 在局部区域连续性上更好；组合损失在整体结构稳定性和小区域恢复之间取得了较好的折中。

\begin{figure}[H]
    \centering
    \begin{subfigure}{0.31\textwidth}
        \includegraphics[width=\textwidth]{../tasks/task3_semantic_segmentation/outputs/0000382_ce.png}
        \caption{Cross-Entropy}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.31\textwidth}
        \includegraphics[width=\textwidth]{../tasks/task3_semantic_segmentation/outputs/0000382_dice.png}
        \caption{Dice}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.31\textwidth}
        \includegraphics[width=\textwidth]{../tasks/task3_semantic_segmentation/outputs/0000382_combo.png}
        \caption{Cross-Entropy + Dice}
    \end{subfigure}
    \caption{样本 \code{0000382} 在三种损失函数下的预测结果}
    \label{fig:segmentation-single-predictions}
\end{figure}

\subsection{结果分析与结论}

综合定量结果和可视化结果，可以得到以下结论：
\begin{itemize}
    \item 仅使用 Cross-Entropy 能够提供稳定的像素级分类能力，但对小类别和边界区域的优化相对不足；
    \item Dice Loss 更直接面向区域重叠优化，因此在类别不平衡场景下表现更强；
    \item Cross-Entropy + Dice 在本实验设置下取得了最高的验证集 \code{mIoU=0.6531}，说明组合损失在稳定性和区域优化之间达到了更优折中。
\end{itemize}

因此，在当前数据划分和训练配置下，\textbf{Cross-Entropy + Dice 是本实验中效果最好的损失函数配置}。
\section{综合讨论}

三类实验体现了不同视觉问题的主要难点。花卉分类实验中，训练集每类只有 10 张图像，模型性能高度依赖预训练特征。ResNet 从零训练与 ImageNet 预训练之间超过 34\% 的差距说明，小样本细粒度分类很难仅依靠目标数据集本身学习出稳健特征。注意力模块能进一步提升 CNN baseline，但收益小于更强 backbone 和更高分辨率输入带来的增益。
...
图像分割实验表明，三种损失函数都能够稳定训练轻量 U-Net，但组合损失在当前配置下取得了最佳综合效果。验证集结果显示，Cross-Entropy、Dice 和 Cross-Entropy + Dice 的最佳 mIoU 分别为 0.6436、0.6477 和 0.6531，其中组合损失最高。这说明仅使用逐像素交叉熵虽然能够提供稳定的分类监督，但引入 Dice 项后，模型在区域重叠和小类别恢复上会进一步受益。

从 per-class IoU 来看，sky、road 和 building 等大面积类别在三种方法下都相对容易，而 mountain 仍然是最难类别。Dice Loss 在 mountain 上优于单独的 Cross-Entropy，说明区域级优化对困难类别有帮助；而 Cross-Entropy + Dice 在 sky、tree、road、grass、water、building 和 foreground\_object 等大多数类别上都取得了最优或接近最优的结果，因此最终整体 mIoU 最高。这个结果也说明，在图像分割任务中，pixel accuracy 往往更容易被大面积类别主导，而 mIoU 和 per-class IoU 更能真实反映不同类别的分割质量。

\section{结论}

本文完成了花卉图像分类、道路车辆检测与跟踪、图像分割三类视觉识别实验。花卉分类中，ConvNeXt-Tiny 384px 在 10 个随机种子上取得 \accpm{95.60}{0.61} 的测试准确率，是本实验中最稳定的分类模型；预训练、注意力机制、输入分辨率和超参数选择均显著影响细粒度分类表现。车辆检测中，YOLOv8s 在验证集上取得 mAP50 0.520，并能结合 ByteTrack 完成跟踪 ID 显示和越线计数，但遮挡仍会造成 ID switch。图像分割中，手写轻量 U-Net 在当前实验设置下分别取得 Cross-Entropy 0.6436、Dice 0.6477 和 Cross-Entropy + Dice 0.6531 的最佳验证集 mIoU，其中最优配置为 Cross-Entropy + Dice，说明组合损失在像素级分类稳定性和区域重叠优化之间达到了更好的平衡。

总体而言，三组实验从分类、检测跟踪和图像分割三个层次展示了深度视觉模型在空间智能问题中的应用方式。预训练特征提供了小样本识别的基础，检测器稳定性决定了跟踪质量，而像素级理解则需要结构设计和损失函数共同配合。

\appendix

\section{最终分类模型训练配置}
\label{app:classification-final-config}
...
\section{基础超参数搜索结果总表}
\label{app:classification-hparam-tables}
